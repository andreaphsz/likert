...
my.likert.bar.plot <- function(likert,
							low.color='#D8B365',
							high.color='#5AB4AC',
	...
	if(!is.null(likert$grouping)) {
		...
		if(centered) {
			...
			p <- p + scale_fill_manual(legend, breaks=names(cols), values=cols) + theme(panel.margin = unit(1, "lines"))
      p <- p + theme(strip.text=element_text(lineheight=1.2))
      if(!is.null(likert$items)) {
        if( dim(likert$items)[2]==1 ) p <- p + theme(strip.text.x=element_blank(), strip.background=element_blank())
      }else{
        if(length(unique(likert$results$Item)) == 1) p <- p + theme(strip.text.x=element_blank(), strip.background=element_blank())
      }
		} else {
			ymin <- 0
			p <- ggplot(results, aes(y=value, x=Group, group=variable))
...
# 			}
		...
		p <- p +
			coord_flip() +	ylab('Prozent') + xlab('') +
			theme(axis.ticks=element_blank(), panel.grid.major.y=element_blank(), panel.grid.major.x = element_line(size=0.1,color="grey50"),panel.grid.minor.x = element_line(size=0.1, color="grey50"),
				  strip.background=element_rect(fill=panel.strip.color,
				  							    color=panel.strip.color), axis.title=element_text(size=8) )
		...
		p <- p +
			coord_flip() + ylab('Prozent') + xlab('') + 
			theme(axis.ticks=element_blank(), axis.title=element_text(size=8))
		if(!missing(group.order)) {
			p <- p + scale_x_discrete(limits=rev(group.order),
